        ...
        <div className="sticky top-36 mb-10 h-max w-96">
          <div className="border p-10 flex flex-col gap-5">
            ...
            <div className="flex justify-between">
              ...
              <h4 className="text-md font-bold text-[#74767e]">
                {gigData.number}
              </h4>
            </div>
            {/* <h6 className="font-medium text-lg">${gigData.price}</h6> */}
            {gigData.userId === userInfo?.id ? (
              ...
              <button
                className="flex items-center bg-[#2499c7] hover:bg-blue-800 text-white py-2 justify-center font-bold text-lg relative rounded"
                // onClick={() => router.push(`/checkout?gigId=${gigData.id}`)}
                onClick={() => window.open(`https://wa.me/54${gigData.number}?text=Me%20contacto%20por%20la%20publicaci%C3%B3n%20en%20Servify`)}
              >
                <span>Contactar</span>
                <BiRightArrowAlt className="text-2xl absolute right-4" />
              </button>

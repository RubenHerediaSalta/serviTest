...
export const addGig = async (req, res, next) => {
  try {
    if (req.files) {
      ...
      if (req.query) {
        const { title, description, category, features, shortDesc, number } =
          req.query;
    ...
    if (req.files) {
      ...
      if (req.query) {
        ...
        oldData?.images.forEach((image) => {
          if (existsSync(`uploads/gigs/${image}`))
            unlinkSync(`uploads/gigs/${image}`);
        });
...
export const checkGigOrder = async (req, res, next) => {
  try {
    // Comentario: Se elimina la verificación de la orden del usuario
    // y siempre se responde con "hasUserOrderedGig: true".
    return res.status(200).json({ hasUserOrderedGig: true });
  } catch (err) {
    console.log(err);
    return res.status(500).send("Internal Server Error");
  }
...
export const addReview = async (req, res, next) => {
  try {
    if (req.userId && req.params.gigId) {
      // Comentario: Se elimina la verificación de la orden del usuario
      // y siempre se procede a agregar la revisión.
      if (req.body.reviewText && req.body.rating) {
        const prisma = new PrismaClient();
        const newReview = await prisma.reviews.create({
          data: {
